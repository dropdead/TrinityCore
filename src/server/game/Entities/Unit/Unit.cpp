...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
        ...
        && (victim->GetTypeId() == TYPEID_PLAYER || !victim->ToCreature()->isWorldBoss()))
    {
        ...
        int32 VictimDefense=victim->GetDefenseSkillValue();
        if (victim->GetTypeId() == TYPEID_PLAYER)
            VictimDefense += victim->ToPlayer()->GetRatingBonusValue(CR_DEFENSE_SKILL);
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31658:
                {
                    *handled = true;
                    ...
                    int32 bp0 = int32(CalculatePctN(this->GetMaxPower(POWER_MANA), spInfo->Effects[0].CalcValue() ));
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        // Glyph of Shadow Solo en shadow form
        case 55689:
            if (GetTypeId() == TYPEID_PLAYER)
                if (Player * plr = this->ToPlayer())
                    if (plr->GetShapeshiftForm() != FORM_SHADOW)
                        return false;
            break;
        case 71761: // Deep Freeze Inmunidad
        {
...
void Unit::SetMinion(Minion *minion, bool apply)
{
    ...
    if (apply)
    {
        ...
        minion->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE);
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (ProcTriggeredList::const_iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
    {
        ...
        // Stealth no es removido si el daño es abosorvido.
