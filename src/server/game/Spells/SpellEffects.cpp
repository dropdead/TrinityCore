        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 54171:                                   //Divine Storm
                {
                    ...
                    {
                        SpellInfo const* spInfo = sSpellMgr->GetSpellInfo(53385);

                        int32 heal = spInfo->Effects[1].CalcValue() * damage / m_UniqueTargetInfo.size() / 100;
        ...
        && effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        ...
        switch (triggered_spell_id)
        {
            ...
            case 18461:
            {
                ...
                unitTarget->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IN_COMBAT); //vanish saca de combate
...
{
    bool fspeed = false;
    if (m_spellInfo->Effects[i].MiscValue)                        // Fixed Time?
        speedZ = float(m_spellInfo->Effects[i].MiscValue)/10;
    else if (m_spellInfo->Effects[i].MiscValueB)                  // Fixed Speed?
    {
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    // Revisa si la piedra tiene cd para usar el comando .st
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties)
{
    ...
    for (uint32 count = 0; count < amount; ++count)
    {
        ...
        if (summon->GetEntry() == 1964) // Force of Nature
        {
