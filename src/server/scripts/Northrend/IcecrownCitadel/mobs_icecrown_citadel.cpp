...
public:
    ...
    struct mob_icecrown_citadel_captain_arnathAI: public mobs_icecrown_citadelAI
    {
        ...
        void EnterCombat(Unit *who)
        {
            ...
            DoCast(me,SPELL_POWER_WORD_SHIELD_ALL);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                if (DoCastTry(me,SPELL_DOMINATE_MIND))
                    Dominate_Mind = 15000;
            ...
            if (Smite <= diff)
            {
                ...
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1,40,false))
                    if (DoCastTry(target,RAID_MODE(SPELL_SMITE_10,SPELL_SMITE_10_HEROIC,SPELL_SMITE_25,SPELL_SMITE_25_HEROIC)))
                        Smite = 15000; // tiempo generico para todas dificultades.
            }else Smite -= diff;

            if (Flash_heal <= diff)
            {
                // Aca como la spell Flash heal se utiliza en dos dificultades usamos RAID_MODE que tomara la dificultad de la insta.
                if (DoCastTry(me,RAID_MODE(SPELL_FLASH_HEAL_MODE_10,SPELL_FLASH_HEAL_MODE_25)))
                    Flash_heal = urand(2000,7000);
            }else Flash_heal -= diff;

            if (Power_word_shield <= diff)
            {
                if (DoCastTry(me,RAID_MODE(SPELL_POWER_WORD_SHIELD_10,SPELL_POWER_WORD_SHIELD_25)))
    ...
    struct mob_icecrown_citadel_captain_brandonAI: public mobs_icecrown_citadelAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                if (DoCastTry(me->getVictim(),SPELL_CRUSADER_STRIKE))
            ...
            if (Hammmer_Betrayal <= diff)
            {
                ...
                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 1,40,false))
                    if (DoCastTry(target,(SPELL_HAMMER_OF_BETRAYAL)))
                        Hammmer_Betrayal = 10000; 
            }else Hammmer_Betrayal -= diff;

            if (Judgement_Of_Command <= diff)
            {
                if (DoCastTry(me->getVictim(),SPELL_JUDGEMENT_OF_COMMAND))
